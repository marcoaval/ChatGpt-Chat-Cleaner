...
I had a lot of random chats mixed in with school, coding, and other important stuff, so I wanted something that could help me find the chats I probably do not need anymore without just deleting everything automatically.



The script looks through the chats that are currently loaded in the ChatGPT sidebar and looks for certain keywords. It then gives you a list so you can review everything before deleting anything.
...
3\. Copy the contents of `chatgpt\_personal\_chat\_cleaner.user.js`
...
For example, personal keywords can include things like:



```javascript

const PERSONAL\_KEYWORDS = \[
...
The script does not know what a chat actually means. It is mostly looking at the title and checking for keywords.
...
If ChatGPT changes its website layout in the future, parts of the script may also stop working and need to be updated.
